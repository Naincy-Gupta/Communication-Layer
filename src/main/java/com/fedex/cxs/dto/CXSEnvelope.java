...
/**
 * This wrapper follows the FedEx CXS Envelope 3.5 format. It also serves as a translation layer between Jackson and the
 * objects that the application uses internally, namely outputs.
 *
 * <code>
 * {
...
public class CXSEnvelope<T extends CXSOutput> {
    ...
    public static <R extends CXSOutput> CXSEnvelope<R> success(final String transactionId, final R output) {
        return new CXSEnvelope<>(transactionId, output);
    }
     ...
     */
    public static CXSEnvelope error(final String transactionId, final String errorCode, final String errorMessage) {
     ...
     * Build a CXSEnvelope that contains errors
     */
    public static CXSEnvelope error(final String transactionId, final CXSError... errors) {
        return new CXSEnvelope<>(transactionId, errors);
    }
